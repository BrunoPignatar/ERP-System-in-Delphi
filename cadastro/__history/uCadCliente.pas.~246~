...
type
  TfrmCadCliente = class(TfrmTelaHeranca)
    ...
    QrySituacaoIDSituacao: TFDAutoIncField;
...
uses
  uDTMConexao, cValidacaoDocumento, cUsuarioLogado, uProVenda, uLogin, cFuncao, uCadUsuario, uPrincipal;
...
begin
  ...
  if oCliente.Seleciona(QryListagem.FieldByName('clienteId').AsInteger) then begin
     ...
     edtDocumento.Text:=oCliente.doc;
...
begin
  ...
  if edtDataNascimento.Date > Date then begin
    ShowMessage('Data inválida');
    edtDataNascimento.Date := date;
    Exit;
  end
  else if YearsBetween(edtDataNascimento.Date, Date) < 18 then
  begin
...
begin
  ...
  if id = 1 then
    ...
    ImageList1.Draw(dbgrdListagem.Canvas, Rect.Left + (Rect.Width div 2) - 8, Rect.Top + (Rect.Height div 2) - 8, imgIndex);
  end
  else
  begin
...
procedure TfrmCadCliente.edtDataNascimentoExit(Sender: TObject);
var Idade: Integer;
begin
  inherited;
  Idade := YearsBetween(Date, edtDataNascimento.Date);
  if IncYear(edtDataNascimento.Date, Idade) > Date then
    Dec(Idade);

  if Idade < 18 then
    ShowMessage('O cliente deve ser de maior');
...
function TfrmCadCliente.ApenasNumeros(const S: string): string;
var
  i: Integer;
begin
  Result := '';
...
function TfrmCadCliente.BuscarCEP(const CEP: string): TJSONObject;
var IdHTTP: TIdHTTP; SSL: TIdSSLIOHandlerSocketOpenSSL; Response: string; CEPLimpo: string;
begin
  Result := nil;
  CEPLimpo := ApenasNumeros(CEP);
