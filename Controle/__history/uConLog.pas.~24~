...
type
  TfrmLog = class(TfrmTelaHerancaConsulta)
    btnPesquisar: TButton;
    QryListagemID: TFDAutoIncField;
    QryListagemDATA_HORA: TSQLTimeStampField;
    QryListagemUSUARIO: TStringField;
...
{$R *.dfm}
 ...
 function TfrmLog.RetornarCampoTraduzido(Campo:String):String;
  var i:Integer;
  begin
  ...
  procedure TfrmLog.ExibirLabelIndice(Campo:string; aLabel:TLabel);
  begin
...
begin
  ...
  begin
      CondicaoSQL := WhereOrAnd+' '+' CAST '+'('+NomeCampo +' as date)' + '='+QuotedStr(mskPesquisa.Text)
  end
...
procedure TfrmLog.grdPesquisaTitleClick(Column: TColumn);
begin
...
procedure TfrmLog.mskPesquisaChange(Sender: TObject);
var Date:TDateTime;
begin
